<template>
	<div :class="jvClass">
		<div v-if="copyable" :class="`jv-tooltip right`">
			<span @click="onCopy" class="jv-button" :class="{ copied }">
				<slot name="copy" :copied="copied"> {{ copiedText }}</slot>
			</span>
		</div>
...
export default {
	...
	methods: {
		...
		onCopy() {
			this.copiedText = this.copiedText == "copy" ? "Copied" : "copy";
			navigator.clipboard.writeText(JSON.stringify(this.value));
			setTimeout(() => (this.copiedText = this.copiedText == "copy" ? "Copied" : "copy"), 1500);
		},
		toggleExpandCode() {
